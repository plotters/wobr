...
final class DecoratorFactories implements DecoratorFactory
{
	...
	private final Map<DecoratorFactory, DecoratorFactory> body = new HashMap<DecoratorFactory, DecoratorFactory>();
	...
	public void add(DecoratorFactory firstDecoratorFactory, List<DecoratorFactory> decoratorFactories)
	{
	...
	public void assertCompleteChain()
	{
		...
		for(DecoratorFactory decorating : body.keySet())
		{
			...
			{
				messages.add(new Message(String.format("decorator factory %s can not be found in the chain", decorating)));
			}
